...
func ShortenURL(c *fiber.Ctx) error {
	...
	body := new(request) //declare a variable of type struct
	if err := c.BodyParser(&body); err != nil { // copy request body into struct variable, a form which golang understands
	...
	defer r2.Close() // whichever function you write in front of defer, will be executed at the end of call stack of this function
	// close the database connection at the end
	val, err := r2.Get(database.Ctx, c.IP()).Result()
	if err == redis.Nil {
		// This user (IP) has accessed for first time, so store his IP, API quota and time limit for it
		...
		// This user (IP) has used the service in the past 30 mins
		val, _ = r2.Get(database.Ctx, c.IP()).Result()
		valInt, _ := strconv.Atoi(val) // valInt is the apiQuota for this IP
		if valInt <= 0 {
	...
	// A single user(IP) can use this service only for 10 times per 30mins, after which it resets
	...
	// Let's give user the power to decide how the url's will look
	...
	// check if the user provided short is already in use by some other user
	if val != "" {
